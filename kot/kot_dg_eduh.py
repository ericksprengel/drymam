...
def is_there_restart():
  si = device.takeSnapshot().getSubImage( (0, 0, 80, 80) )
  si.writeToFile("si.png", "png")
  print(device.takeSnapshot().getRawPixel(85, 1020))
#  if 85,1020 == rgb(253,45,46)
  return False

def geinti(time, initial, final):
  return (final - initial)*time/(const.TOTAL_TRIES - 1.0) + initial
...
counter = 0
#while True:
for j in range(const.TOTAL_TRIES):
  if must_exit():
    sys.exit(0)
  print("iteration: %d" % (j) )
  iterate(j)
